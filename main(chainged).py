...
def detect_outliers(df, nameOfFeature, drop=False):
    ...
    step = iqr(valueOfFeature) * 1.5  # Using scipy's iqr function
    
    # Find outliers using built-in Pandas filtering
    outliers = df[(valueOfFeature < Q1 - step) | (valueOfFeature > Q3 + step)].index.tolist()
    
    # If drop is True, remove outliers
    if drop:
...
insulin_outliers = detect_outliers(data, 'Insulin', drop=True)
